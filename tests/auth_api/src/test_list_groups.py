...
def test_group_list():
    ...
    ans = requests.get(f"http://{AUTH_API_HOST}/groups/")
...
def test_create_delete_group():
    ...
    # )
    ans = requests.post(
        f"http://{AUTH_API_HOST}/users/login?login=admin&password={os.getenv('ADMIN_PASSWORD')}"
    )
    assert ans.status_code == 200
    data = ans.json()
...
def test_nobody_create_group():
    ...
    # )
    ans = requests.post(
        f"http://{AUTH_API_HOST}/users/login?login=nobody&password={os.getenv('NOBODY_PASSWORD')}"
    )
    assert ans.status_code == 200
    data = ans.json()
    token = data["access_token"]
...
def test_users_paginated(seven_little_guys):
    ...
    # )
    ans = requests.get(
        f"http://{AUTH_API_HOST}/groups/{gid}/users/?page_size=3&page_number=1"
    )
    assert ans.status_code == 200
    data = ans.json()
    assert isinstance(data, list)
    ...
    # )
    ans = requests.get(
        f"http://{AUTH_API_HOST}/groups/{gid}/users/?page_size=3&page_number=2"
    )
    assert ans.status_code == 200
    data = ans.json()
    assert isinstance(data, list)
    ...
    # )
    ans = requests.get(
        f"http://{AUTH_API_HOST}/groups/{gid}/users/?page_size=3&page_number=3"
    )
    assert ans.status_code == 200
    data = ans.json()
    assert isinstance(data, list)
